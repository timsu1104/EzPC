...
CompResultType sort(std::vector<IntegerArray>& data, std::vector<int>& plain_key, int size, bool record, bool acc) {
  CompResultType result;
  result.recording = record;
  bitonic_sort(data, plain_key, 0, size, acc, acc, result);
  return result;
}
CompResultType sort(IntegerArray& data, std::vector<int>& plain_key, int size, bool record, bool acc) {
  std::vector<IntegerArray> wrapper{data};
  auto result = sort(wrapper, plain_key, size, record, acc);
  data = wrapper[0];
  return result;
}
